...
export default function CookieBanner() {
  ...
  const decline = () => {
    try {
      localStorage.setItem(
        "cookie-consent",
        JSON.stringify({ v: 0, embeds: false, t: Date.now() })
      );
    } catch {}
    setOpen(false);
  };
      ...
      <div className="mx-auto max-w-3xl rounded-2xl border border-primary/15 bg-white/90 backdrop-blur supports-[backdrop-filter]:bg-white/80 p-4 shadow-lg ring-1 ring-primary/10">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm text-gray-800">
            ...
            contenidos embebidos (p. ej., Google Maps/Form). Consulta la{" "}
            <a
              href="#/legal/cookies"
          ...
          <div className="flex gap-2 sm:shrink-0">
            ...
            <button
              onClick={accept}
              className="inline-flex items-center justify-center rounded-lg bg-gradient-to-r from-primary to-primary-700 px-4 py-2 text-sm font-semibold text-white hover:from-primary-700 hover:to-primary-800"
            >
              Aceptar
            </button>
            <button
      ...
      {prefsOpen && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center">
          ...
          <div
            role="dialog"
            aria-modal
            className="relative mx-4 w-full max-w-md rounded-2xl bg-white p-5 shadow-2xl ring-1 ring-primary/15"
            tabIndex="-1"
            ref={prefsRef}
          >
            ...
            <div className="mt-4 space-y-3">
              ...
              <label className="flex items-start gap-3">
                ...
                />
                <span>
                  <span className="font-medium">Contenidos de terceros</span>{" "}
                  (Mapas/Formularios de Google)
                </span>
              </label>
            </div>
              ...
              <button
                onClick={() => {
                  try {
                    localStorage.setItem(
                      "cookie-consent",
                      JSON.stringify({ v: 1, embeds, t: Date.now() })
                    );
                  } catch {}
                  setPrefsOpen(false);
                  setOpen(false);
